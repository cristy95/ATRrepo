...
--controller
create or replace
	function set_hon_stud(p_stud_id char(9), 
				p_dissertation text, 
				p_special_project text, 
				p_thesis_title text)
	returns text as
...
begin
	...
	if v_stud_id isnull then
		insert into honored_students(stud_id, 
						dissertation, 
						special_project, 
						thesis_title) 
		values (p_stud_id, p_dissertation, p_special_project, p_thesis_title);
	else
		update honored_students
		set dissertation = p_dissertation, 
			special_project = p_special_project, 
			thesis_title = p_thesis_title
		where stud_id = p_stud_id;
	end if;
